...
import { animatSE } from "./se-animat.lib.js";
import { pathSE } from "./se-path.lib.js";
let isStats = false;
...
window.onload = () => {
    ...
    function testThings() {
        ...
        setInterval(() => a3E.innerHTML = String(app.ticker.FPS), 300);
        setInterval(() => document.getElementById('a2').innerHTML = String(SE.getFpsRate()), 300);
    }
};
export let SE = {
    Vector: Vector,
    ...
    control: {
        hideAllAnimats() {
            ...
            let names = Object.keys(animatSE.pixiItemClips);
            for (let i = 0; i < names.length; i++) {
                let qA = animatSE.pixiItemClips[names[i]].queArray;
        ...
        stopAllAnimats() {
            for (let i = 0; i < animatSE.particleShellBox.length; i++)
                animatSE.particleShellBox[i].lifeMax = 0;
            animatSE.particleShellBox = [];
        }
    },
    animat: animatSE,
    tool: {
