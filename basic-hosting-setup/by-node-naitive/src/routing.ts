...
export let routing = {
    ...
    createHttpServer() {
        ...
        let server = http.createServer(function (request, response) {
            ...
            if (request.method === 'GET') {
                ...
                FileService.getDataObjToSendP(filePath).then(
                    // 找到
                    (dataObj: FileService.dataObj) => {
                        let mimeType = FileService.getMimeTypeFromExt(dataObj.fileExt);
    ...
    /**　將url分解成所需資料。 */
    handleQueryString(url: string): { [key: string]: any } {
        let parsedObj: { [key: string]: any } = {};
        if (url.indexOf('?') != -1) {
            //之後去分割字串把分割後的字串放進陣列中
            let ary1 = url.split('?');
            //此時ary1裡的內容為：
